...
import { getLunchtime, getSuggestion, sleep } from '../utils/suggest';
...
const initCommands = (app: App) => {
  app.command('/lunchtime', async ({ command, ack, say, payload }) => {
    ...
    const minutes = command.text;
    await ack();
    if (command.text === '' || parseInt(command.text, 10) === null) {
      await say('❌❌❌ Please place time... ❌❌❌');
    } else {
      await say('Lunchtime!');
      resetSession();
      ...
      await say({ blocks });
      const countDown = moment().add(5, 'seconds').format('HH:mm:ss');
      await sleep(parseInt(minutes, 10) * 1000);
      // ADD BLOCK BELOW
      await say(`⏰ ${countDown} ⏰ We are going to:`);
    }
  });
